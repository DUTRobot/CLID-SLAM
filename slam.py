...
from utils.tools import freeze_model, get_time, save_implicit_map, setup_experiment, split_chunks, remove_gpu_cache, \
    create_bbx_o3d
...
def run_slam(config_path=None, dataset_name=None, sequence_name=None, seed=None):
    ...
    for frame_id in tqdm(range(dataset.total_pc_count)):
        ...
        if frame_id > 0:
            if config.track_on:
                cur_pose_torch, valid_flag = iekfom.update_iterated(dataset.cur_source_points)
        ...
        if config.o3d_vis_on:
            ...
            if vis_visualize_on:
                ...
                if vis_mesh_on and (frame_id == 0 or frame_id == last_frame or (
                    ...
                    if not vis_global_on:  # only build the local mesh
                        chunks_aabb = split_chunks(global_neural_pcd_down, dataset.cur_bbx,
                                                   vis_mesh_mc_res_m * 100)  # reconstruct in chunks
                        cur_mesh = mesher.recon_aabb_collections_mesh(chunks_aabb, vis_mesh_mc_res_m, None, True,
                        ...
                        chunks_aabb = split_chunks(global_neural_pcd_down, aabb,
                                                   vis_mesh_mc_res_m * 300)  # reconstruct in chunks
                        cur_mesh = mesher.recon_aabb_collections_mesh(chunks_aabb, vis_mesh_mc_res_m, None, False,
                                                                      config.semantic_on, config.color_on,
                                                                      filter_isolated_mesh=True,
                                                                      mesh_min_nn=vis_mesh_min_nn)
                        # cur_sdf_slice = None
                ...
                if vis_sdf_on and (frame_id == 0 or frame_id == last_frame or (frame_id + 1) % vis_sdf_freq_frame == 0):
                    ...
                    cur_sdf_slice_h = mesher.generate_bbx_sdf_hor_slice(vis_sdf_bbx, dataset.cur_pose_ref[
                        2, 3] + vis_sdf_slice_height, vis_sdf_res_m, True, -sdf_bound,
                ...
                if dataset.cur_frame_o3d is not None:
                    packet_to_vis.add_scan(np.array(dataset.cur_frame_o3d.points, dtype=np.float64),
                                           np.array(dataset.cur_frame_o3d.colors, dtype=np.float64))

                if cur_mesh is not None:
                    packet_to_vis.add_mesh(np.array(cur_mesh.vertices, dtype=np.float64), np.array(cur_mesh.triangles),
                                           np.array(cur_mesh.vertex_colors, dtype=np.float64))

                if cur_sdf_slice is not None:
                    packet_to_vis.add_sdf_slice(np.array(cur_sdf_slice.points, dtype=np.float64),
                                                np.array(cur_sdf_slice.colors, dtype=np.float64))

                if pool_pcd is not None:
                    packet_to_vis.add_sdf_training_pool(np.array(pool_pcd.points, dtype=np.float64),
        ...
        cur_frame_process_time = np.array(
            [T2 - T1, T3 - T2, T5 - T4, T6 - T5, T4 - T3])  # loop & pgo in the end, visualization and I/O time excluded
