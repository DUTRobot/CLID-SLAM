...
class Config:
    def __init__(self):
        ...
        self.tracking_method: str = "ours"
        self.mapping_method: str = "ours"
        ...
        self.global_loop_on: bool = False  # global loop detection using context descriptor
        ...
        self.publish_np_map_down_rate_list = [11, 23, 37, 53, 71, 89, 97, 113, 131,
                                              151]  # prime number list, downsampling for boosting neural point map pubishing speed
    ...
    def load(self, config_file):
        ...
        if "setting" in config_args:
            ...
            self.tracking_method = config_args["setting"].get("tracking_method", "ours")
            self.mapping_method = config_args["setting"].get("mapping_method", "ours")
        ...
        if "process" in config_args:
            ...
            self.dynamic_sdf_ratio_thre = config_args["process"].get("dynamic_sdf_ratio_thre",
                                                                     self.dynamic_sdf_ratio_thre)
            self.dynamic_certainty_thre = config_args["process"].get("dynamic_certainty_thre",
                                                                     self.dynamic_certainty_thre)
        ...
        if "sampler" in config_args:
            ...
            self.free_sample_end_dist_m = config_args["sampler"].get("free_sample_end_dist_m",
                                                                     self.surface_sample_range_m * 4.0)  # this value should be at least 2 times of surface_sample_range_m
        ...
        if "neuralpoints" in config_args:
            ...
            self.buffer_size = int(float(config_args.get("neuralpoints", {}).get("buffer_size", self.buffer_size)))
            self.num_nei_cells = config_args["neuralpoints"].get("num_nei_cells", self.num_nei_cells)
            self.layer_norm_on = config_args["neuralpoints"].get("layer_norm_on", self.layer_norm_on)
            ...
            self.local_map_travel_dist_ratio = config_args["neuralpoints"].get("local_map_travel_dist_ratio",
                                                                               self.local_map_travel_dist_ratio)
            self.window_radius = config_args["neuralpoints"].get("window_radius", self.window_radius)
            self.local_map_radius = config_args["neuralpoints"].get("local_map_radius", self.local_map_radius)
        ...
        if "loss" in config_args:
            ...
            self.ekional_loss_on = config_args["loss"].get("ekional_loss_on",
                                                           self.ekional_loss_on)  # use ekional loss (norm(gradient) = 1 loss)
        ...
        if self.track_on:
            if self.color_on:
                ...
                if self.photometric_loss_on:
                    self.photometric_loss_weight = float(
                        config_args["tracker"].get("photo_weight", self.photometric_loss_weight))
                self.consist_wieght_on = config_args["tracker"].get("consist_wieght", self.consist_wieght_on)
            self.T_imu_lidar = torch.tensor(config_args["tracker"].get("T_imu_lidar", self.T_imu_lidar))
            ba_value = config_args.get("tracker", {}).get("ba")
            if ba_value is not None:
                self.bg = torch.tensor(config_args["tracker"].get("ba", self.ba), device=self.device,
                                       dtype=self.tran_dtype)
            bg_value = config_args.get("tracker", {}).get("bg")
            if bg_value is not None:
                self.bg = torch.tensor(config_args["tracker"].get("bg", self.bg), device=self.device,
                                       dtype=self.tran_dtype)
            ...
            self.measurement_noise_covariance = config_args["tracker"].get("measurement_noise_covariance",
                                                                           self.measurement_noise_covariance)
            self.bias_noise_covariance = config_args["tracker"].get("bias_noise_covariance", self.bias_noise_covariance)
        ...
        if self.pgo_on:
            ...
            self.context_virtual_side_count = config_args["pgo"].get("virtual_side_count",
                                                                     self.context_virtual_side_count)
            ...
            self.min_loop_travel_dist_ratio = config_args["pgo"].get("min_loop_travel_ratio",
                                                                     self.min_loop_travel_dist_ratio)
            self.loop_dist_drift_ratio_thre = config_args["pgo"].get("max_loop_dist_ratio",
                                                                     self.loop_dist_drift_ratio_thre)
        ...
        if "optimizer" in config_args:
            ...
            self.init_iter_ratio = config_args["optimizer"].get("init_iter_ratio",
                                                                self.init_iter_ratio)  # iteration count ratio for the first frame (a kind of warm-up) #iter = init_iter_ratio*iter

            self.new_sample_ratio_more = config_args["optimizer"].get("new_sample_ratio_more", self.new_sample_ratio_more)
