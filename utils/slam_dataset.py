...
class SLAMDataset(Dataset):
    ...
    def preprocess_frame(self):
        ...
        if frame_id == 0:  # initialize the first frame, no tracking yet
            ...
            if self.config.tracking_method == "ours":
                ...
                self.last_odom_tran_ = inv(self.last_pose_ref) @ pose_propagation
            else:
                # pose initial guess
                ...
                if self.config.uniform_motion_on and not self.lose_track:
                    ...
                    cur_pose_init_guess = self.last_pose_ref @ self.last_odom_tran
        ...
        if frame_id > 0:
            ...
            if self.config.tracking_method == "pin slam":  # 我们的定位方法
            ...
            elif self.config.tracking_method == "ours":  # 我们的定位方法
                if self.config.deskew and not self.lose_track:
                    self.cur_source_points = deskewing(
                        self.cur_source_points,
                        cur_source_ts,
                        torch.tensor(
                            self.last_odom_tran_, device=self.device, dtype=self.dtype
                        )
                    )  # T_last<-cur
